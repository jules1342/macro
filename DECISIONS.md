...
- **2026-09-06, the Weight tab is Trends.** It holds weight, calories, protein and a maintenance-over-time chart. Each TDEE dot is a 42-day trailing window and the line is a 14-day average of the dots. Julian found 28 days with no smoothing still spiky on real data: one odd weigh-in at either end of the window moves the whole estimate. The five-window breakdown is collapsed by default. The range selector has a 1Y option and stays pinned to the top while scrolling, so it can be changed from any chart.
