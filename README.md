...
The app keeps `Macros/App Data/macro.json` in your Drive. That layout, one top-level folder per app with an "App Data" folder inside it, is the convention for all of Julian's apps. It is not a shared "App Data" parent, because the `drive.file` scope only lets an app see files it created, so a second app would be blind to a shared folder and would create a duplicate. Sync overwrites it with the current device state. Restore pulls it back and replaces this device's data, so a new phone can be set up from it. Sync is manual: tap it after a change worth keeping.
